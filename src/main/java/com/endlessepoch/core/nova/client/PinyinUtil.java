...
public final class PinyinUtil {
    ...
    private static final PinIn ctx;
    static {
        PinIn c;
        try { c = new PinIn(); c.config().fZh2Z(true).fSh2S(true).fCh2C(true).fAng2An(true).fIng2In(true).fEng2En(true).accelerate(true).commit(); }
        catch (Exception ignored) { c = null; }
        ctx = c;
    }

    public static boolean matches(String query, String target) {
        if (ctx == null || query == null || target == null || query.length() < 2) return false;
